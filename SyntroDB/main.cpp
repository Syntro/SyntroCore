...
void loadSettings(QStringList arglist)
{
	...
	if (nSize == 0) {
		...
		for (int index = 0; index < SYNTRODB_MAX_STREAMS; index++) {
			...
			settings->setValue(SYNTRODB_PARAMS_CREATE_SUBFOLDER, SYNTRO_PARAMS_TRUE);
			...
			settings->setValue(SYNTRODB_PARAMS_ROTATION_TIME, "2");
			settings->setValue(SYNTRODB_PARAMS_ROTATION_SIZE, "256");
		}
		settings->endArray();
	}
