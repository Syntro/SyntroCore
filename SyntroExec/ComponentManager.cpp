...
void ComponentManager::startComponent(ManagerComponent *component)
{
	...
	if (component->monitored) {
		...
		component->helloStateString = "No hello yet";
		component->helloSeen = false;
	}
}

void ComponentManager::checkHello(ManagerComponent *component)
{
	...
	if (m_hello->findComponent(&helloEntry, &(component->UID))) {
		...
		component->helloSeen = true;						// now seen the hello
...
void ComponentManager::startModeKillAll()
{
	...
	UID = m_componentData.getMyUID();						// preset the UID
	for (i = 0; i < SYNTRO_MAX_COMPONENTSPERDEVICE; i++) {
		if (i == INSTANCE_EXEC)
			continue;										// don't kill SyntroExec!
